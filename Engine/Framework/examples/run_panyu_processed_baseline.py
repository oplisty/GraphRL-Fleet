...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run baseline simulation on processed real Panyu map")
    ...
    parser.add_argument("--task-mode", choices=["uniform_nodes", "hotspot_nodes"], default="uniform_nodes")
    ...
    parser.add_argument("--out", default="Framework/output/panyu_processed")
    args = parse_args_with_yaml(parser)

    env = build_environment(args)
    summary = env.run()

    out_dir = Path(args.out) / f"panyu_processed_{args.scheduler}_v{args.vehicles}_t{args.tasks}"
    env.export_logs(str(out_dir))

    print("Panyu Processed Simulation Summary")
